 ...
 *
 * <p>Implementations of this interface are expected to be thread-safe and can be safely accessed by multiple concurrent
 * threads.
 *
 * @param <K> the type of keys maintained by this cache
 * @param <V> the type of mapped values. If and only if a cache declares a nullable value type, then its loading
 *     function may return null values, and the cache will return those null values to callers. (Null values are still
 *     never <i>stored</i> in the cache.)
 */
public interface Cache<K, V extends Object> {
     ...
     *
     * <p>If the specified key is not already associated with a value, attempts to compute its value using the given
     ...
     *
     * <p><b>Warning:</b> as with {@link CacheLoader#load}, {@code mappingFunction} <b>must not</b> attempt to update
    ...
    /**
     * Discards all entries in the cache. The behavior of this operation is undefined for an entry that is being loaded
     * (or reloaded) and is otherwise not present.
     *
     * @return this
     */
    @NonNull
    ...
    /**
     * Discards any cached values for the {@code keys}. The behavior of this operation is undefined for an entry that is
     * being loaded (or reloaded) and is otherwise not present.
     *
