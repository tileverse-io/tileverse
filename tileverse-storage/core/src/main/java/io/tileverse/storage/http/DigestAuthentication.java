        ...
        if (params == null) {
            try {
                ...
                if (params != null) {
                    ...
                    log.warn("Could not get digest authentication parameters for {}", uri);
                    return requestBuilder;
                }
            } catch (Exception e) {
                log.warn("Failed to fetch digest authentication parameters", e);
                return requestBuilder;
            }
        }
    ...
    private DigestParams fetchDigestParams(HttpClient httpClient, URI uri) {
        try {
            ...
            if (response.statusCode() == 401) {
                ...
                log.warn("Expected 401 response for digest auth, got {}", response.statusCode());
                return null;
            }
