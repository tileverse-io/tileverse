...
public final class SeekableByteChannelImageInputStream extends ImageInputStreamImpl {
    ...
    private SeekableByteChannelImageInputStream(SeekableByteChannel channel) {
        ...
        if (channel instanceof SelectableChannel) {
            log.warn("Channel is a SelectableChannel and may switch to non-blocking mode, "
    ...
    private void warnIfZeroBytesRead(int bytesRead) {
        if (bytesRead == 0
                && channel instanceof SelectableChannel selectableChannel
                && !selectableChannel.isBlocking()) {
            log.warn("""
                SelectableChannel returned 0 bytes and is in non-blocking mode. \
